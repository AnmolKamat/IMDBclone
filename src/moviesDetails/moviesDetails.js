...
function MoviesDetailsPage() {
  ...
  const widt = `${Math.floor(movieDetails.vote_average * 10)}%`;
  ...
  useEffect(() => {
    const newCombinedCrew = combineCrewMembers(crewDetails);
    setCombinedCrew(newCombinedCrew);
  }, [crewDetails]);

  if (isLoadingMovie || isLoadingCast || isLoadingCrew) {
        ...
        <div className="poster-container">
          ...
          <div className="title-container">
            ...
            <p className="date">
              ...
              {Object.values(movieDetails.genres).map((genre) => (
                <span className="genre">{genre.name}</span>
              ))}
              <span className="duration">{`${Math.floor(
                +movieDetails.runtime / 60
              )}.${+movieDetails.runtime % 60}h`}</span>
            </p>
